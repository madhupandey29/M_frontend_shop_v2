...
export default function ClientProductDetails({ slug, serverProduct }) {
  const { data, isError, isLoading } = useGetSingleNewProductQuery(slug, { 
    skip: !slug,
    // Use server data as initial data if available
    initialData: serverProduct ? { data: serverProduct } : undefined
  });

  let content;
  if (isLoading)        content = <ProductDetailsLoader loading />;
  else if (isError)     content = <ErrorMsg msg="There was an error" />;
  else if (!data)       content = <ErrorMsg msg="No product found!" />;
  else                  content = (
    <ProductDetailsArea product={mapBackendProductToFrontend(data.data)} />
  );
