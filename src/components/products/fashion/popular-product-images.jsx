          ...
          <Link href={`/fabric/${slug}`} key={item._id} style={{ textDecoration: 'none', display: 'block' }}>
            <div style={rowStyle}>
              ...
              <div style={detailsStyle}>
                <div style={nameStyle} title={item.name}>{item.name?.slice(0, 30) || 'Product'}</div>
                <div style={infoStyle}>
                  {item.gsm && <>GSM: {item.gsm}</>}
                  {item.gsm && item.salesPrice && <span> | </span>}
                  {item.salesPrice && <span style={priceStyle}>${item.salesPrice}</span>}
                </div>
              </div>
            </div>
          </Link>
...
export default PopularProductImages; 